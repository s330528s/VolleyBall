...
public class PlayerControllerServer : MonoBehaviour
{
    ...
    void TryJump()
    {
        if (Player_IsGround && JumpKey)
        {
            ...
            // Ting
            PlayerParam.Server.Player_Force_Y = yForce;
        }
        else
        {
            PlayerParam.Server.Player_Force_Y = 0;
        }
    }
    ...
    void MovementX()
    {
        ...
        // Ting
        PlayerParam.Server.Player_Force_X = xForce * horizontalDirection;
    }


    void Player1Position()
    {
        // PlayerParam.Player_Pos.X = player1Transform.position.x;
        // PlayerParam.Player_Pos.Y = player1Transform.position.y;

        // Ting
        PlayerParam.Server.Player_UpdateSt = true;
        PlayerParam.Server.Player_Pos_X = player1Transform.position.x;
        PlayerParam.Server.Player_Pos_Y = player1Transform.position.y;
        PlayerParam.Server.Player_Velocity_X = player1Rigidbody2D.velocity.x;
        PlayerParam.Server.Player_Velocity_Y = player1Rigidbody2D.velocity.y;
    }
    
    void Player2Position()
    {
        // player2Transform.position = new Vector2(-PlayerParam.Player_Pos.X, PlayerParam.Player_Pos.Y);

        // Ting
        if ( PlayerParam.Client.Player_UpdateSt == true )
        {
            player2Transform.position = new Vector2(PlayerParam.Client.Player_Pos_X, PlayerParam.Client.Player_Pos_Y);
            player2Rigidbody2D.velocity = new Vector2(PlayerParam.Client.Player_Velocity_X, PlayerParam.Client.Player_Velocity_Y);

            PlayerParam.Client.Player_UpdateSt = false;
        }

        // player2Transform.Translate(new Vector2(PlayerParam.Client.Player_Velocity_X, PlayerParam.Client.Player_Velocity_Y) * Time.deltaTime, Space.World);
        
        
        // player2Rigidbody2D.AddForce(new Vector2(PlayerParam.Client.Player_Force_X, PlayerParam.Client.Player_Force_Y);
    }
    // ============================= Player Position =============================
    //////////////////////////////// Player Operation ////////////////////////////////
    ...
    void BallForce()
    {
        if ( BallTransform.position.x < 0 )
        {
            ...
            {
                //Ting
                PlayerParam.Server.Ball_Force_UpdateSt = true;
                PlayerParam.Server.Ball_Force_X = BallVecForce.x * 30;
                PlayerParam.Server.Ball_Force_Y = BallVecForce.y * 30;

                ballRigidbody2D.AddForce(new Vector2(PlayerParam.Server.Ball_Force_X, PlayerParam.Server.Ball_Force_Y));
            }

            // BallPosSet
            PlayerParam.Server.Ball_UpdateSt = true;

            PlayerParam.Server.Ball_Pos_X = BallTransform.position.x + ( ballRigidbody2D.velocity.x * Time.deltaTime );
            PlayerParam.Server.Ball_Pos_Y = BallTransform.position.y + ( ballRigidbody2D.velocity.y * Time.deltaTime );

            PlayerParam.Server.Ball_Velocity_X = ballRigidbody2D.velocity.x;
            PlayerParam.Server.Ball_Velocity_Y = ballRigidbody2D.velocity.y;
        }
        else
        {
            // TIng
            // From Client.
            if ( PlayerParam.Client.Ball_UpdateSt == true ) // BallPosUpdate
            {
                BallTransform.position = new Vector2(PlayerParam.Client.Ball_Pos_X, PlayerParam.Client.Ball_Pos_Y);
                ballRigidbody2D.velocity = new Vector2(PlayerParam.Client.Ball_Velocity_X, PlayerParam.Client.Ball_Velocity_Y);

                PlayerParam.Client.Ball_UpdateSt = false;
            }
            
            if ( PlayerParam.Client.Ball_Force_UpdateSt == true )
            {
    ...
    void Start()
    {
        ...
        PlayerParam.Client.Player_Pos_X = Player2_InitPos.x;
        PlayerParam.Client.Player_Pos_Y = Player2_InitPos.y;

        Ball_in_OurGround = true;
    ...
    void Set_InitPos(bool ball_in_ourGround) // Server.
    {
